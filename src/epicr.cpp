...
namespace epicr
{
		...
		std::unordered_map<std::string, ingredient> ingredients)
	{
		...
		{
			if (pair.first == to_lower(ingredientName))
				return true;
		}
		return false;
	}
	...
	{
		cmd_args args = {filename, E_STYLE_FANCY, "dist", E_US_NONE, false, false};
		return parse_recipe(args);
	}

	parse_ret parse_recipe_silent(std::string filename)
	{
		cmd_args args = {filename, E_STYLE_FANCY, "dist", E_US_NONE, true, false};
	...
	rcp_ret ingredient_verify_recipe(recipe *recipe)
	{
		...
		rcp_ret ret = {recipe, ingrvisit.has_error, ingrvisit.error, " IngVis: "};
		return ret;
	}
	...
	void parse_cmd_args(int argc, char **argv)
	{
		...
		epicr::cmd_args CMD_ARGS = {"", E_STYLE_FANCY, concat_output_dir("dist"), E_US_NONE, false, false};
		for (int i = 0; i < argc; i++)
		{
