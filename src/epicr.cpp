...
namespace epicr
{
	...
	{
		cmd_args args = {filename, E_OS_BASIC, "dist", E_US_NONE, false, false};
		return parse_recipe(args);
	}

	parse_ret parse_recipe_silent(std::string filename)
	{
		cmd_args args = {filename, E_OS_BASIC, "dist", E_US_NONE, true, false};
	...
	void parse_cmd_args(int argc, char **argv)
	{
		...
		epicr::cmd_args CMD_ARGS = {"", E_OS_BASIC, concat_output_dir("dist"), E_US_NONE, false, false};
		for (int i = 0; i < argc; i++)
		{
